...
from .api import LATEST_API_VERSION, URI_PREFIX_API, URI_PREFIX_API_DOCS, api_router
...
def root():
    return RedirectResponse(
        url=f"/{URI_PREFIX_API}/{LATEST_API_VERSION}/{URI_PREFIX_API_DOCS}"
    )
...
for route in api_router.routes:
    table.append(
        [f"/{URI_PREFIX_API}{route.path}", route.name, ",".join(route.methods)]
    )
